  ...
  $(".remove_one").click(function (event) {
    ...
    $.ajax({
      ...
      success: function (cart) {
        if (quantity.text() > 1) {
          key = cart["cart"][id + "-" + size];
          actualQuantity = key["quantity"];
    ...
    success: function (json) {
      ...
      $("#code")
        ...
        .prepend(
          `<img src="${
            json.image
          }"><div class="info"><p class="item-name">${gettext("Coupon")} - ${
            json.code
          }</p><p class="quantity">${gettext(trans1)} ${
            json.discount
          }% ${gettext(trans)}</p><p class="price">- ${
            json.get_discount
          } $</p></div>`
        );
      $("#message-coupon").hide();
